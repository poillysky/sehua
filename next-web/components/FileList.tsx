...
type FileItem = Ed2kResourceProps["files"][0] & {
  index: number | string;
  path: string;
  extension?: string;
  size?: number | string;
...
/**
 * Renders a file list for an ED2K resource.
 */
export default function FileList({
  ...
  resource: Ed2kResourceProps;
  highlight?: string | string[];
  max?: number;
}) {
  const t = useTranslations();
  const list = max > 0 ? resource.files.slice(0, max) : resource.files;
  ...
  return (
    <ul>
      ...
      {max > 0 && resource.files.length > max && (
        <Link
          isExternal
          className="text-sm italic text-gray-500"
          href={`/detail/${hexToBase64(resource.hash)}`}
        >
          {t("Search.more_files", {
            count: resource.files.length - max,
          })}
        </Link>
      )}
    </ul>
