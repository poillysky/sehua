...
[![Version](https://img.shields.io/badge/version-1.1.16-0ea5e9?style=flat-square)](./VERSION)
...
镜像由 GitHub Actions 构建并推送 **Docker Hub**（及 GHCR）；NAS **只 pull，不本地 build**。发版递增叠加（`1.0.1` … `1.1.16`），历史标签保留，`latest` 始终指向当前版。
...
| 服务 | 镜像 |
|------|------|
| 后端 | [`poillysky/sehuatang-backend:1.1.16`](https://hub.docker.com/r/poillysky/sehuatang-backend) |
| 管理 | [`poillysky/sehuatang-admin:1.1.16`](https://hub.docker.com/r/poillysky/sehuatang-admin) |
| 搜索 | [`poillysky/sehuatang-search:1.1.16`](https://hub.docker.com/r/poillysky/sehuatang-search) |
...
```text
ghcr.io/poillysky/sehuatang-backend:1.1.16
ghcr.io/poillysky/sehuatang-admin:1.1.16
ghcr.io/poillysky/sehuatang-search:1.1.16
```
...
下一版 **1.1.17**：同步改 `VERSION`、`deploy/docker-compose.nas.yml` 镜像标签、workflow `RELEASE_TAG`，提交并打 `v1.1.17`。
