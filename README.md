...
当前版本：**1.0.5**（见 [`VERSION`](./VERSION)）。发版按 `1.0.1` → `1.0.2` → `1.0.3` → `1.0.4` → `1.0.5` → … **递增叠加**，旧版本标签保留。
...
| 服务 | 当前标签 |
|------|----------|
| 后端 | [`poillysky/sehuatang-backend:1.0.5`](https://hub.docker.com/r/poillysky/sehuatang-backend) |
| 管理 | [`poillysky/sehuatang-admin:1.0.5`](https://hub.docker.com/r/poillysky/sehuatang-admin) |
| 搜索 | [`poillysky/sehuatang-search:1.0.5`](https://hub.docker.com/r/poillysky/sehuatang-search) |

账号：https://hub.docker.com/u/poillysky

每次发版会推送 **版本号**（如 `1.0.5`）和 **`latest`（始终指向当前最新版）**。历史版本号会留在 Hub（`1.0.1`、`1.0.2`、`1.0.3`、`1.0.4`、`1.0.5`…），NAS Compose 用固定版本号钉住。

发版前若只要清掉「还没版本号时」的杂标签：Hub → Tags → 删除多余短 SHA 等即可，**不要删已发布的版本号标签**。

### GHCR（可选）

- `ghcr.io/poillysky/sehuatang-backend:1.0.5`
- `ghcr.io/poillysky/sehuatang-admin:1.0.5`
- `ghcr.io/poillysky/sehuatang-search:1.0.5`
...
- 下次发 **1.0.6**：改 `VERSION`、`deploy/docker-compose.nas.yml` 镜像标签、workflow 的 `RELEASE_TAG`，提交并打 `v1.0.6`
- Hub 上会留下 `1.0.1`、`1.0.2`、`1.0.3`、`1.0.4`、`1.0.5`…；`latest` 指向最新一次发版
