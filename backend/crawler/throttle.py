...
class AutoThrottle:
    ...
    _stop: bool = False
    # 本轮首次 sleep 减半（列表扫完/开爬后第一帖）
    _first_sleep_pending: bool = True
    ...
    def configure(
        ...
        *,
        ...
        self.base_delay = max(0.3, float(base_delay or 1.0))
        ...
        self._first_sleep_pending = True
        # 不在此清除 _stop：避免配置刷新冲掉正在进行的手动停止

    def min_delay(self) -> float:
        """连续成功时允许的下限（不低于 0.3，且不低于 base 的一半）。"""
        return max(0.3, float(self.base_delay) * 0.5)
    ...
    def _adjust(self) -> None:
        ...
        if rate <= 0:
            # 全成功：逐步降到 floor，不必卡在 base
            self.current_delay = max(floor, self.current_delay * 0.9)
            return
        factor = 1.0 + rate * 6.0
    ...
    async def sleep(self) -> None:
        delay = float(self.current_delay or 0)
        if self._first_sleep_pending:
            self._first_sleep_pending = False
            delay = delay * 0.5
        await self.sleep_for(delay)
    ...
    def status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "fetch_delay_min": round(self.min_delay(), 2),
