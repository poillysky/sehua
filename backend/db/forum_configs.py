    ...
    forums = [
        {
            ...
            "policies": [
                "Discuz 专用爬虫：配置 / Cookie / 开关与 2048 互不共用；连续调度只跑调度焦点",
                "启用多板轮转：一轮内当前板扫列表 → 启用板合计抓帖；深扫触底后再切 active_board_fid",
                "分区：综合讨论区（95/141/142）+ 原创BT电影白名单；跳过 fid=148 鲍鱼直播盒子及未登记区（在线视频/原档/图片/文学）",
                "综合讨论：95 仅 typeid=716 情色分享（ED2K）；141 按分类子版、发帖满 3 天才入队（ED2K）；142 转帖按分类（磁力）",
                "原创BT：列表按发帖时间；正文磁力优先，无链再种子附件；同帖多链可 ×N 入库",
                "判帖：软文/安全壳可浏览器重读；正文已有楼层时嵌套 static/safe 不算壳；可从【哈希校验】补磁力",
                "附件：登录提示/日限 50/只有特定用户可下载 → 占位「附件无权（占位入库）」；账号重爬用独立账号 Cookie jar",
            ],
        },
        {
            "id": FORUM_2048_ID,
            ...
            "policies": [
                "PHPWind 独立爬虫：配置 / Cookie（cookies_2048.json）/ 开关与色花堂互不共用",
                "进站：发布页（fby.*）解析当日 BBS 链 → 记 preferred_entry_url；列表/帖链都挂在该 BBS 根",
                "白名单分区：新片合集(3) / 分区资源(4,5,13,15,16,18,67) / BT磁力(343,195,318)；主链磁力优先，无则 ED2K",
                "列表 thread.php?fid= · 帖子 read.php?tid=；跳过回家指南/地址发布器/在线影院广告等版务置顶",
                "fid=5 日本騎兵、fid=13 歐美新片：需论坛 Cookie；售价 0 先尝试解锁，失败占位留给账号重爬；售价>0 跳过",
                "判帖：#read_tpc 含售价后电驴；软壳但已有正文则继续解析；网盘链不入库；空 Cookie=游客不复活旧 jar",
            ],
        },
    ]
    return {
